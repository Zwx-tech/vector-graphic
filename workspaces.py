...
class Workspace(object):
    ...
    def translate_to_pixels(self, vector: Vector = None, x=None, y=None) -> Vector or tuple[int, int]:
        ...
        if (x is not None) and (y is not None) and (Vector is None):
            return round(self.multiplier * self.zoom * x + self.x_offset), \
                   round(self.multiplier * self.zoom * y + self.y_offset)

        return Vector(
            round((self.multiplier * self.zoom * vector.x) + self.x_offset),
            round((self.multiplier * self.zoom * vector.y) + self.y_offset)
        )
    ...
    def translate_to_units(self, vector: Vector = None, x=None, y=None) -> Vector or tuple[int, int]:
        ...
        if (x is not None) and (y is not None) and (Vector is None):
            return (x - self.x_offset) / self.multiplier / self.zoom, \
                   (y - self.y_offset) / self.multiplier / self.zoom

        return Vector(
            (vector.x - self.x_offset) / self.multiplier / self.zoom,
            (vector.y - self.y_offset) / self.multiplier / self.zoom
        )
